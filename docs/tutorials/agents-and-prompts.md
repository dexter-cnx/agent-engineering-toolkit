...
ถ้ารู้ stack แล้ว ให้เติม overlay-specific notes แบบสั้น ๆ ต่อท้าย เช่น:

- Flutter: `mobile-flutter` overlay
- Web frontend: `web-frontend` overlay
- Services: `backend-node` หรือ `python-service` overlay

## ตัวอย่าง AGENTS.md ตาม platform

### Flutter

ถ้าคุณอยากได้ AGENTS.md สั้น ๆ แบบที่ใช้จริงได้เลย:
...
- ห้ามใส่ business logic หรือ API call ใน widget
...
- ใช้ Riverpod สำหรับ state
- ใช้ go_router สำหรับ routing
- อย่าใช้ setState หรือ global mutable state เป็นหลัก

## 6. Development Workflow

1. อ่าน `AGENTS.md`
2. อ่าน `docs/prompt-pipeline.md`
3. ใช้ prompts ตามลำดับ
4. verify ก่อน finalize
5. update project memory เมื่อมี decision สำคัญ
...
- data fetching ต้องไม่อยู่ใน leaf component
- page logic ต้องบาง
...
1. อ่าน `AGENTS.md`
2. เลือก overlay
3. ใช้ lifecycle prompts
4. verify ด้วย lint/build/test
5. update memory เมื่อมี decision ถาวร
```
...
- transport layer ต้องบาง
- orchestration layer จัดการ flow
- persistence layer จัดการ storage
- adapter layer ห่อ external providers
...
2. เลือก overlay
3. ใช้ lifecycle prompts
4. verify ด้วย lint/test/startup
5. update memory เมื่อมี decision ถาวร
```

### Flutter

- ใช้ `mobile-flutter` overlay
- ถ้าเป็น tutorial Flutter ให้ AGENTS.md ใส่ state management, routing, และ localization rule แบบสั้น ๆ
- แยก business logic ออกจาก widget boundary
- ระบุ test command ที่ repo นี้ใช้จริง

### Web frontend

- ใช้ `web-frontend` overlay
- ถ้าเป็น tutorial web frontend ให้ AGENTS.md ใส่ page, component, state, service rule แบบสั้น ๆ
- เขียน design-system หรือ component convention เท่าที่จำเป็น
- ระบุ lint/build/test command ให้ครบ

### Services

- ใช้ `backend-node` หรือ `python-service` overlay ตาม stack
- ถ้าเป็น tutorial service ให้ AGENTS.md ใส่ route/handler, service/orchestration, repository, adapter rule แบบสั้น ๆ
- ไม่ให้ transport logic ไหลเข้า domain
- ระบุ startup, test, smoke command ให้ครบ
