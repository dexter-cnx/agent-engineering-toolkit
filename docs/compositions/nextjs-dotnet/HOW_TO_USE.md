...
## What it is
This is the reference path for building a Next.js frontend with an ASP.NET Core backend using the modular overlay system.
...
## Example stack map
- SaaS admin panel -> use all four overlays
- frontend prototype -> use frontend overlays first, then add backend overlays when the API is known
...
3. Scaffold the ASP.NET Core solution and API project structure.
4. Write the first API contract in `backend-common`.
5. Add the matching frontend feature map.
6. Implement the contract in backend-dotnet.
...
4. Implement JWT and refresh flow in `backend-dotnet`.
...
4. Implement EF Core and validation in `backend-dotnet`.
...
- Check that ASP.NET Core specifics live only in backend-dotnet.
